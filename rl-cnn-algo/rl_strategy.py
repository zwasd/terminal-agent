...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        self.defence_agent = Agent(fname=(defence_agent_file, defence_memo_file, defence_epsilon_file), 
                                   ...
                                   memory_size=1000, batch_size=64, epsilon_min=0.01, epsilon_dec=0.9998,
                                   input_shape=(28,28,6), epsilon=1.0)
    ...
    def retrieve_state(self, game_state):
        ...
        Returns a 3D np array with shape 28x28x6
        The two dimensions 28x28 represent every cell on the board,
        and the last dimension 6 depends on the cell:
          - If the cell is in arena bound, it is a onehot encoded vector for the unit type
          - If the cell is not in arena bound, the last dimension will store my_health, enemy_health, turn number, SP, MP
        
        """
        state = np.zeros((28,28,6))

        # Get state of every position on the board
        for x in range(28):
            for y in range(28):
                if game_state.game_map.in_arena_bounds((x, y)):
                    # If (x,y) is in arena bound and there's unit in (x,y),
                    # set the corresponding onehot index of that unit to 1
                    if len(game_state.game_map[x, y]) > 0:
                        unit_index = UNIT_TYPE_TO_INDEX[game_state.game_map[x, y][0].unit_type]
                        state[x,y,unit_index] = 1
                else:
                    # If (x,y) is not in arena bound, use (x,y) to store other info as below
                    state[x,y,0] = game_state.my_health
                    state[x,y,1] = game_state.enemy_health
                    state[x,y,2] = game_state.turn_number
                    state[x,y,3] = game_state.MP
                    state[x,y,4] = game_state.SP
