...
class AlgoStrategy(gamelib.AlgoCore):
    def __init__(self, defence_agent, attack_agent, load=False):
        super().__init__()
        ...
        if load:
            defence_agent.load_model("defence_agent.h5") # For loading in old models
            attack_agent.load_model("attack_agent.h5")
    ...
    def on_turn(self, turn_state):
        ...
        if len(self.previous_states) > 0:
            # Calculate reward
            reward = self.reward_function(self.previous_state, current_state)
